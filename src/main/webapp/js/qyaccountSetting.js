var qyaccountSetting = {};
$(function () {
	//点击后弹出密码修改
	$("#mdPassport").click(function () { //点击后弹出
	...
	//绑定邮箱点击弹出
	$('#bindEmail,#mdEmail').click(function () { //点击后弹出
  ...
  //修改或绑定邮箱，修改密码，ajax
    $(".confirmBtn").click(function(){
    	...
    	if($("#dyna_flag").val()=="1"){
    		if(qyaccountSetting.checkEmail()){
    			qyaccountSetting.bindEmail();
    		}
    	}else if($("#dyna_flag").val()=="0"){
    		if(qyaccountSetting.checkPwd()){
    			qyaccountSetting.editPwd();
    		}
    	}
    });
 ...
 * 绑定邮箱
 */
qyaccountSetting.bindEmail = function(){
	var time = new Date().getTime();
	var data = $("#ipt_email").serialize();
	$.ajax({
		type:"POST",
		url:"entpuser/bindemail",
		data:data+"&time="+time,
 ...
 * 修改密码
 */
qyaccountSetting.editPwd = function(){
	var time = new Date().getTime();
	var data = "entppassword="+$("#ipt_currpwd").val()+"&newpassword="+$("#ipt_newpwd").val();
	$.ajax({
		type:"POST",
		url:"entpuser/editpwd",
		data:data+"&time="+time,
		success:function(msg){
			if(msg=="success"){
 ...
 * 验证邮箱
 */
qyaccountSetting.checkEmail = function(){
	if($("#ipt_email").val()==""){
 ...
 * 验证密码
 */
qyaccountSetting.checkPwd = function(){
