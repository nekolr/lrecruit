...
<body>
	...
	<div class="main">
		<div class="content">
			<div class="left">
				...
				<div class="gradeBshort on" data-url="entpuser/gocvmanager">待筛选<div class="cvNum seling"></div></div>
				...
				<div class="gradeBshort" data-url="entpuser/goluqu">已录取</div>
				<div class="gradeBshort" data-url="entpuser/gonot">不合适简历</div>
			</div>
			<div class="right">
			<form action="entpuser/gocvmanager" id="fm_cv">
			...
			<div class="listTab" id="listTab1" style="display: block;">
				<c:choose>
					<c:when test="${conditionMap.status==1}">
						<span data-val="1" class="on">未读 (<em>${should}</em>)</span>
						<span data-val="2">已阅 (<em>${readed}</em>)</span>
					</c:when>
					<c:when test="${conditionMap.status==2}">
						<span data-val="1">未读 (<em>${should}</em>)</span>
						<span data-val="2" class="on">已阅 (<em>${readed}</em>)</span>
					</c:when>
				</c:choose>
			</div>
			...
			<c:if test="${pagger.dataList!=null}">
				...
				<c:forEach items="${pagger.dataList}" var="item">
					<div class="cv" data-cvid="${item.cvid}" data-jobid="${item.jobid}" data-sendtime="${item.sendtime}" data-url="gocv">
						<div class="til_cv">
							...
							<div class="opt_box">
								<div class="rt_opt">
								    <p>
										...
										<span data-opt="5">已录取</span> -->
										<span data-opt="6" class="on" data-recordid="${item.recordid}">不合适</span>
										<!-- <span data-opt="7">删除</span> -->
									</p>
								</div>
							    <div class="lf_opt">
									<p>
										...
										<span data-opt="1" class="on contactU" data-recordid="${item.recordid}">感兴趣</span>
				...
				laypage({
				  ...
				  jump: function(e, first){ //触发分页后的回调
				    if(!first){ //一定要加此判断，否则初始时会无限刷新
				      ...
				      location.href = "entpuser/gocvmanager?"+fm;
				    }
				  }
				});
			</script>
	...
	<div class="pop-info normal" style="display:none">
		<div class="pop-body">
			<div class="pop-title">
